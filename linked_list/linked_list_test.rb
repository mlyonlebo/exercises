...
class LinkedListTest < Minitest::Test
  ...
  #initialize Element with a value of one; retrieve that value with #datum
  def test_element_datum
    element = Element.new(1)
    assert_equal 1, element.datum
  end

  #element has a value, tail
  def test_element_tail
    element = Element.new(1)
    assert element.tail?
  end

  #element has no value next (i.e. no linked value)
  def test_element_next_default
    element = Element.new(1)
    assert_nil element.next
  end

  #initialize new element with a second arg. with a default value of nil
  ...
  #peek shows the value of the last pushed element
  ...
  #if the list is empty, peek returns nil
  def test_peeking_at_empty_list
    list = SimpleLinkedList.new
    assert_nil list.peek
  end

  #list.head returns last pushed element
  ...
  #list.push(1) instantiates a new Element object (Element.new(1)), adds this to collection
  #list.push(2) instnatiates a new Element object, (Element.new(2), list.head)
